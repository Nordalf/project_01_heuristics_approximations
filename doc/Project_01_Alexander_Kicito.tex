...
Moving point E which is the closest point in route \(j\)to \(i\) is resulted in different tour between visiting E before \(B\)or after. Thus, it is necessary to compare cost between these two sub cases before perform moving operation as showing in Line 9 of Algorithm 5. 
...
\begin{algorithm}[hbt!]
	...
	\Require{$is\_capacity\_reached\_when\_move(route, A)$: returns True if capacity of given route is satisfy after add point A}
	\Require{$is\_capacity\_reached\_when\_move(route1, route2, A, B)$: returns True if capacity of given routes is satisfy swap  point A of route1 with point B of route2}
	\Require{$elim\_empty\_routes$: returns list of routes where there empty route(contain only depot point) is removed}
	...
	\While{\texttt{$\var{improvements}$}}
		\ForAll{$i$, $j$ in $routes\_permutation(len(route))$ }
			...
			\If{\textbf{not} $ is\_capacity\_reached\_when\_move(\var{routes[j]}, \var{closest\_from\_i\_j})$}
			...
			\ElsIf{\textbf{not} $is\_capacity\_reached\_when\_move(\var{routes[i]}, \var{closest\_from\_j\_i})$}
			...
			\ElsIf{\textbf{not} $ is\_capacity\_reached\_when\_swap(\var{routes[i]}, \var{routes[j]}, \var{closest\_from\_j\_i})$}
...
\begin{algorithm}[hbt!]
	...
	\State $\var{A}, \var{B}, \var{C} \gets \var{route\_i[i\_to\_j -1]}, \var{route\_i[i\_to\_j]}, \var{route\_i[i\_to\_j+1 \% len(route\_i)]}$

	\State $\var{D}, \var{E}, \var{F} \gets \var{route\_j[j\_to\_i -1]}, \var{route\_j[j\_to\_i]}, \var{route\_j[j\_to\_i+1 \% len(route\_j)]}$
	...
	\If{$\var{old\_distance\_i} + \var{old\_distance\_j} > \var{new\_distance\_i} + \var{new\_distance\_j}$}
...
\begin{algorithm}[hbt!]
	...
	\If{$\var{new\_dist\_before\_B} < {new\_dist\_after\_B}$}
		...
		\If{$\var{old\_distance\_i} + \var{old\_distance\_j} > \var{new\_dist\_before\_B} + \var{new\_distance\_j}$}
			move point E from route j to route i before B
		\EndIf
	\Else

		\If{$\var{old\_distance\_i} + \var{old\_distance\_j} > \var{new\_dist\_after\_B} + \var{new\_distance\_j}$}
