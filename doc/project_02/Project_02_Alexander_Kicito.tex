...
Our Simulated Annealing algorithm does run for a long time with an exponential complexity. From the Table \ref{table:cost}, we are getting close in some cases, 
but our initial from our heuristic project part 1 Cluster algorithm(CHH), where it grows routes from furthest point from depot and using nearest addition heuristic approach to form a tour, is still doing better in most cases. We concluded that allowing more possibility on developing neighbor selection is not benefit total cost much. Ideas on improvement for neighbor selection is to create smarter algorithm to pick neighbor such as select exchange neighbors based on it's proximity could allow Simulated Annealing discover better local optimum sooner in each iteration. Compared to our not-finished, and bad performed, Ant Colony Optimization algorithm, 
the Simulated Annealing algorithm is performing better.
