                ...
                for path, subdirs, files in os.walk('../data'):
                    for name in files:
                        if fnmatch.fnmatch(name, "*.xml"):
                            ...
                            splitted_name = re.compile("^[a-zA-Z]+").findall(name)
                            ...
                            for (ls_idx, l_alg) in enumerate(ls_alg):
                                with open('../results/'+ls_alg_name[ls_idx]+'_'+h_alg_name[idx]+'.dat', "a") as ls_fh:
                                    ...
                                    try:
                                        ...
                                        print("timeout")
                                        sol = e.solution
                                    ls_fh.write("{} {} {}".format(splitted_name[0], sol.cost(), round(t0,2)))
                                    ls_fh.write("\n")
                                    ls_fh.close()
                filehandle.close()
                boxplotter('../results/'+h_alg_name[idx])
                for ls_idx in range(len(ls_alg_name)):
                    boxplotter('../results/'+ls_alg_name[ls_idx]+'_'+h_alg_name[idx])
